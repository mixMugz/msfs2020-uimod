...
// Hidden UI code insert.
var mKey = 8; // Backspace keybind (you can change it).
if (localStorage.getItem('mToggle') === null) localStorage.setItem('mToggle', 'true');
window.document.addEventListener("keyup", function (e) {
    if (e.keyCode == mKey) {
        setTimeout(function() { 
            window.document.documentElement.style.setProperty('--tgl-block', (localStorage.getItem('mToggle') == 'true') ? 'block' : 'none');
            window.document.documentElement.style.setProperty('--tgl-flex', (localStorage.getItem('mToggle') == 'true') ? 'flex' : 'none');
        }, 50);
    }
});
function mToggler() {
